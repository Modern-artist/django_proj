...
def club_event(name):
    ...
    a = cursor.fetchone()
    while(a):
        b={'id':a[0],'name':a[2],'start':a[3],'end':a[4],'club':a[5],'genre':a[6],'eligibility':a[7],'description':a[8],'prize':a[10], 'link':a[11], 'date':a[13], 'poster':a[1], 'desc':a[9]}
        c.append(b)
        a = cursor.fetchone()
    return c
...
def get_det(id):
    ...
    a = cursor.fetchone()
    param={'id':a[0],'name':a[2],'start':a[3],'end':a[4],'club':a[5],'genre':a[6],'eligibility':a[7],'desc':a[9],'prize':a[10], 'link':a[11], 'date':a[13], 'poster':a[1]}
    return param

def update_event(name, start, end, club, genre, eligibility, description, prize, link, id):
    cursor=mydb.cursor()
    cursor.execute('UPDATE event SET name=%s, start=%s, end=%s, club=%s, genere=%s, eligibility=%s, short_desc=%s, prize=%s, link=%s where id=%s', (name, start, end, club, genre, eligibility, description, prize, link, id))
...
def club_home(request):
    ...
    id2=request.POST.get('update','none')
    if id != 'none':
        delete(id)
    if id2 != 'none':
        return render(request,'edit.html',get_det(id2))
    if not login:
...
def update(request):
    ...
    link=request.POST.get('link','')
    print("hello")
    update_event(name, start, end, club, genre, eligibility, description, prize, link, id)
    return redirect('c_home')
